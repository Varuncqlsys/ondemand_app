...
public class AvailableJobsAdapter extends RecyclerView.Adapter<AvailableJobsAdapter.MyViewHolder> implements Filterable {
    private Context context;
    SavePref savePref;
    ArrayList<SellerAvailableJobsResponse> availableJobsResponseArrayList;
    ArrayList<SellerAvailableJobsResponse> searchList;
    private AvailableJobsFilter mFilter;
    ProvideQuotesFragment provideQuotesFragment;

    public AvailableJobsAdapter(Context context, ArrayList<SellerAvailableJobsResponse> availableJobsResponseArrayList, ProvideQuotesFragment provideQuotesFragment) {
    ...
    @Override
    public AvailableJobsAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView;
        ...
        return new AvailableJobsAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(AvailableJobsAdapter.MyViewHolder holder, final int position) {
        ...
        if (savePref.getUserType().equals("1")) {
            ...
            } else if (availableJobsResponseArrayList.get(position).getJob_type().equals(context.getResources().getString(R.string.carpenter_id))) {
                holder.image.setImageDrawable(context.getResources().getDrawable(R.drawable.carpenter_seller));
            } else if (availableJobsResponseArrayList.get(position).getJob_type().equals(context.getResources().getString(R.string.ac_id))) {
                holder.image.setImageDrawable(context.getResources().getDrawable(R.drawable.ac_seller));
            } else if (availableJobsResponseArrayList.get(position).getJob_type().equals(context.getResources().getString(R.string.satellite_id))) {
                holder.image.setImageDrawable(context.getResources().getDrawable(R.drawable.satellite_seller));
            } else if (availableJobsResponseArrayList.get(position).getJob_type().equals(context.getResources().getString(R.string.painter_id))) {
        ...
        holder.parent_layout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                provideQuotesFragment.startActivityForResult(intent, 303);
    ...
    public Filter getFilter() {
        if (mFilter == null) {
            mFilter = new AvailableJobsAdapter.AvailableJobsFilter();
        }
        return mFilter;
    }
    ...
    private class AvailableJobsFilter extends Filter {
        @Override
